...
export default {
  ...
  methods: {
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          axios.get('http://localhost:9090/moveout/moveoutSearch', {params: _this.ruleForm}).then(function (resp) {
    ...
    page(currentPage) {
      ...
      if (_this.ruleForm.value === '') {
        axios.get('http://localhost:9090/moveout/moveoutList/' + currentPage + '/' + _this.pageSize).then(function (resp) {
          _this.tableData = resp.data.data.data
          _this.total = resp.data.data.total
        })
      } else {
  ...
  created() {
    const _this = this
    axios.get('http://localhost:9090/moveout/moveoutList/1/' + _this.pageSize).then(function (resp) {
      _this.tableData = resp.data.data.data
      _this.total = resp.data.data.total
    })
  }
