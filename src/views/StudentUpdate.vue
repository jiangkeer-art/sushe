...
export default {
  ...
  methods: {
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          axios.put('http://localhost:9090/student/update', _this.ruleForm).then(function (resp) {
            if (resp.data.code === 0) {
  ...
  created() {
    ...
    axios.get('http://localhost:9090/student/findById/' + _this.$route.query.id).then(function (resp) {
